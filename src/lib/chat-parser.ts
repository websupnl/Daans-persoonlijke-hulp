...
export function parseIntent(input: string): ParsedIntent {
  ...
  // "Boodschappen koffie melk" or "Boodschappen: koffie melk" at start of message
  const groceryStartMatch = text.match(/^boodschappen[:\s]+(.+)/i)
  if (groceryStartMatch) {
    return {
      intent: 'grocery_add',
      confidence: 0.92,
      params: { title: groceryStartMatch[1].trim() },
      raw: text,
    }
  }
