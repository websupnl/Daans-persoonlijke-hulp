...
>(({ children, className, asChild = false, ...props }, ref) => {
  ...
  return (
    <DropdownMenuPrimitive.Trigger ref={ref} asChild {...props}>
      <button className={cn("group relative flex items-center justify-center overflow-hidden rounded-xl transition-all outline-none", className)} {...events}>
        <RippleLayer rippleRef={rippleRef} pressed={pressed} variant="trigger" />
        <span ref={surfaceRef as React.RefObject<HTMLSpanElement>} className="absolute inset-0 z-0" />
        <div className="pointer-events-none relative z-10 flex h-full w-full items-center justify-center gap-[inherit]">
          {children}
...
>(({ className, sideOffset = 8, children, ...props }, ref) => {
  ...
  const staggeredChildren = React.Children.map(children, (child, index) => {
    if (React.isValidElement(child)) {
      return React.cloneElement(child, { style: { ...child.props.style, "--m3-stagger": index } as React.CSSProperties });
    }
    return child;
  });
  ...
  return (
    <DropdownMenuPrimitive.Portal>
      <DropdownMenuPrimitive.Content
        ...
        className={cn(
          "m3-content z-50 overflow-hidden rounded-xl border border-border/20 bg-popover/95 py-0 text-popover-foreground shadow-[0px_8px_32px_rgba(0,0,0,0.12)] outline-none backdrop-blur-xl",
...
>(({ className, inset, children, delayDuration = 250, enterAnimation = true, asChild = false, ...props }, ref) => {
  ...
  const baseClassName = cn(
    "group relative flex min-h-[48px] cursor-pointer select-none items-stretch overflow-hidden rounded-none px-0 text-sm font-medium tracking-[0.01em] outline-none transition-colors",
  ...
  if (asChild && React.isValidElement(children)) {
    return (
      ...
      >
        {React.cloneElement(children as React.ReactElement<any>, {
          children: (
            <div ref={(node) => { (surfaceRef as any).current = node; }} className={cn("relative flex flex-1 items-center px-4", inset && "pl-12")}>
...
>(({ className, ...props }, ref) => (
  <DropdownMenuPrimitive.Separator
    ref={ref}
    className={cn("m3-item-enter my-0.5 h-[1px] w-full bg-gradient-to-r from-transparent via-border to-transparent opacity-80", className)}
    {...props}
  />
));
...
>(({ className, inset, ...props }, ref) => (
  <DropdownMenuPrimitive.Label
    ref={ref}
    className={cn("m3-item-enter px-5 py-4 text-[10px] font-black uppercase tracking-[0.15em] text-primary/80", inset && "pl-12", className)}
    {...props}
  />
));
...
const DropdownMenuPage = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & { id: string }>(
  ({ id, children, className, ...props }, ref) => {
    ...
    const staggeredChildren = React.Children.map(children, (child, index) => {
      if (React.isValidElement(child)) {
        return React.cloneElement(child, { style: { ...child.props.style, "--m3-stagger": id === "main" ? index : index + 1 } as React.CSSProperties });
      }
      return child;
    });
    ...
    return (
      <div
        ...
        className={cn(
          "absolute left-0 top-0 w-full py-0 transition-all duration-[350ms] ease-[cubic-bezier(0.2,0,0,1)]",
          isActive ? "pointer-events-auto translate-x-0 scale-100 opacity-100" : isLeft ? "pointer-events-none -translate-x-[20%] scale-[0.98] opacity-0" : "pointer-events-none translate-x-[20%] scale-[0.98] opacity-0",
          className
        )}
        {...props}
      >
...
>(({ targetId, children, ...props }, ref) => {
  ...
  return (
    ...
    >
      {children}
      <ChevronRight className="ml-auto h-4 w-4 text-muted-foreground opacity-70" />
    </DropdownMenuItem>
  );
});
