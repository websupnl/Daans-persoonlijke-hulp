...
export async function GET(req: NextRequest) {
  try {
    ...
    conditions.push(completed === '1' ? 't.completed = 1' : 't.completed = 0')
    conditions.push(`t.workspace = $${i++}`)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const todo = await queryOne(
      `INSERT INTO todos (title, description, category, priority, due_date, project_id, contact_id, recurring, workspace)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
       RETURNING *`,
      [title.trim(), description || null, category || 'overig', priority || 'medium', due_date || null, project_id || null, contact_id || null, recurring || null, workspace]
    )
