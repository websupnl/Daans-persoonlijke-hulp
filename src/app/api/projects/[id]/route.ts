...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (Number.isNaN(id)) return jsonFail('PROJECT_ID_INVALID', 'Ongeldig project id', 400, undefined, _req)
    ...
    if (!project) return jsonFail('PROJECT_NOT_FOUND', 'Niet gevonden', 404, undefined, _req)
    return jsonOk({ project, todos, notes, worklogs, finance }, undefined, _req)
  } catch (error: unknown) {
    return jsonFail('PROJECT_GET_FAILED', 'Kon project niet ophalen', 500, error, _req)
  }
}
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const id = parseInt(params.id)
    if (Number.isNaN(id)) return jsonFail('PROJECT_ID_INVALID', 'Ongeldig project id', 400, undefined, req)
    ...
    return jsonFail('PROJECT_UPDATE_FAILED', 'Kon project niet bijwerken', 500, error, req)
  }
}
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return jsonOk({ message: 'Project verwijderd' }, undefined, _req)
  } catch (error: unknown) {
    return jsonFail('PROJECT_DELETE_FAILED', 'Kon project niet verwijderen', 500, error, _req)
  }
}
